...
export default function Header() {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50">
      {/* Emergency top bar */}
      <div className="bg-brand-orange text-white text-sm font-body">
        <div className="max-w-7xl mx-auto px-4 py-2 flex items-center justify-between">
          <span className="font-semibold">⚡ 24/7 Emergency Callouts — We&apos;re Always Available</span>
          <a
      ...
      {/* Main nav */}
      <nav
        className={`bg-brand-black transition-shadow duration-300 ${
          scrolled ? "shadow-2xl" : ""
        }`}
      >
        ...
        {menuOpen && (
          <div className="lg:hidden bg-brand-dark border-t border-gray-800">
            <div className="px-4 py-4 flex flex-col gap-1">
              ...
              <a
                href={`tel:${CONTACT.phoneTel}`}
                className="flex items-center gap-2 text-white font-heading font-bold text-lg mt-4"
              >
                <Phone size={20} className="text-brand-orange" />
                {CONTACT.phone}
              </a>
              <Link
                href="/contact"
                onClick={() => setMenuOpen(false)}
                className="mt-3 bg-brand-orange text-white font-heading font-bold px-5 py-3 rounded text-center text-sm uppercase tracking-wide"
              >
                Get a Free Quote
              </Link>
            </div>
